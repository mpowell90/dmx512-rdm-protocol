...
pub fn rdm_parameter(args: TokenStream, input: TokenStream) -> TokenStream {
    let input: DeriveInput = parse_macro_input!(input);
    let name = &input.ident;
    ...
    let command_class = match command_class_expr {
        ...
        None => return syn::Error::new_spanned(
            name, 
            "Missing required argument: 'command_class'"
        ).to_compile_error().into(),
    };
    ...
    let expanded = quote! {
        ...
        impl rdm_parameter_traits::RdmParameterCodec for #name {
            const COMMAND_CLASS: u8 = #command_class;
            ...
            fn encode_parameter_data(&self, buf: &mut [u8]) -> Result<usize, rdm_parameter_traits::ParameterCodecError> {
                let size = self.size_of_parameter_data();
            ...
            fn decode_parameter_data(buf: &[u8]) -> Result<Self, rdm_parameter_traits::ParameterCodecError> {
                let mut offset = 0;
