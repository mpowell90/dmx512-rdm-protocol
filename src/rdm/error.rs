    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidMessageLength(length) => write!(
                f,
                "Invalid message length: {length}, must be >= 24 and <= 255"
            ),
            Self::InvalidChecksum(checksum, expected) => {
                write!(f, "Invalid checksum: {checksum}, expected: {expected}")
            }
            ...
            Self::UnsupportedParameter(command_class, parameter_id) => write!(
                f,
                "Unsupported Parameter, CommandClass: {command_class}, ParameterId: {parameter_id}"
            ),
            Self::InvalidParameterDataLength(length) => write!(
                f,
                "Invalid parameter data length: {length}, must be >= 0 and <= 231"
            ),
            ...
            Self::InvalidBrokerState(broker_state) => {
                write!(f, "Invalid BrokerState: {broker_state}")
            }
            Self::InvalidDiscoveryState(discovery_state) => {
                write!(f, "Invalid DiscoveryState: {discovery_state}")
            }
            Self::InvalidEndpointMode(endpoint_mode) => {
                write!(f, "Invalid EndpointMode: {endpoint_mode}")
            }
