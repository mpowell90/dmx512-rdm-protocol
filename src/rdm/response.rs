use super::{
    bsd_16_crc, device::DeviceUID, CommandClass, ParameterId, ProductCategory, ResponseType,
};
#[derive(Debug)]
...
impl From<&[u8]> for DiscMuteResponse {
...
impl TryFrom<&[u8]> for Response<DiscMuteResponse> {
    type Error = &'static str;

    fn try_from(packet: &[u8]) -> Result<Self, Self::Error> {
        Ok(Response::parse_packet(
            &packet[..=23],
            Some(DiscMuteResponse::from(
                &packet[24..=(24 + (packet[23] - 1) as usize)],
            )),
        ))
    }
}
...
impl From<&[u8]> for DiscUnmuteResponse {
    fn from(bytes: &[u8]) -> Self {
        let binding_uid = if bytes.len() > 2 {
            Some(DeviceUID::from(&bytes[2..]))
        } else {
...
impl TryFrom<&[u8]> for Response<DiscUnmuteResponse> {
    type Error = &'static str;

    fn try_from(packet: &[u8]) -> Result<Self, Self::Error> {
        Ok(Response::parse_packet(
            &packet[..=23],
            Some(DiscUnmuteResponse::from(
                &packet[24..=(24 + (packet[23] - 1) as usize)],
            )),
        ))
    }
...
impl From<&[u8]> for ProxiedDeviceCountResponse {
    fn from(bytes: &[u8]) -> Self {
...
impl TryFrom<&[u8]> for Response<ProxiedDeviceCountResponse> {
    type Error = &'static str;

    fn try_from(packet: &[u8]) -> Result<Self, Self::Error> {
        Ok(Response::parse_packet(
            &packet[..=23],
            Some(ProxiedDeviceCountResponse::from(
                &packet[24..=(24 + (packet[23] - 1) as usize)],
            )),
        ))
    }
...
impl From<&[u8]> for ProxiedDevicesResponse {
    fn from(bytes: &[u8]) -> Self {
...
impl TryFrom<&[u8]> for Response<ProxiedDevicesResponse> {
    type Error = &'static str;

    fn try_from(packet: &[u8]) -> Result<Self, Self::Error> {
        Ok(Response::parse_packet(
            &packet[..=23],
            Some(ProxiedDevicesResponse::from(
                &packet[24..=(24 + (packet[23] - 1) as usize)],
            )),
        ))
    }
...
impl From<&[u8]> for DeviceInfoResponse {
    fn from(bytes: &[u8]) -> Self {
        DeviceInfoResponse {
...
impl TryFrom<&[u8]> for Response<DeviceInfoResponse> {
    type Error = &'static str;

    fn try_from(packet: &[u8]) -> Result<Self, Self::Error> {
        Ok(Response::parse_packet(
            &packet[..=23],
            Some(DeviceInfoResponse::from(
                &packet[24..=(24 + packet[23] as usize)],
            )),
        ))
    }
}
...
impl From<&[u8]> for SoftwareVersionLabelResponse {
    fn from(bytes: &[u8]) -> Self {
        SoftwareVersionLabelResponse {
            software_version_label: String::from_utf8_lossy(bytes).to_string(),
        }
    }
}

impl TryFrom<&[u8]> for Response<SoftwareVersionLabelResponse> {
    type Error = &'static str;

    fn try_from(packet: &[u8]) -> Result<Self, Self::Error> {
        Ok(Response::parse_packet(
            &packet[..=23],
            Some(SoftwareVersionLabelResponse::from(
                &packet[24..=(24 + packet[23] as usize)],
            )),
        ))
    }
...
pub struct SupportParametersResponse {
    pub supported_parameters: Vec<ParameterId>,
}

impl From<&[u8]> for SupportParametersResponse {
    fn from(bytes: &[u8]) -> Self {
        SupportParametersResponse {
            supported_parameters: bytes.chunks(2).map(ParameterId::from).collect(),
        }
    }
}

impl TryFrom<&[u8]> for Response<SupportParametersResponse> {
    type Error = &'static str;

    fn try_from(packet: &[u8]) -> Result<Self, Self::Error> {
        Ok(Response::parse_packet(
            &packet[..=23],
            Some(SupportParametersResponse::from(
                &packet[24..=(24 + (packet[23] - 1) as usize)],
            )),
        ))
    }
...
impl From<u8> for IdentifyDeviceResponse {
    fn from(byte: u8) -> Self {
        IdentifyDeviceResponse {
            is_identifying: byte != 0,
        }
    }
}

impl TryFrom<&[u8]> for Response<IdentifyDeviceResponse> {
    type Error = &'static str;

    fn try_from(packet: &[u8]) -> Result<Self, Self::Error> {
        Ok(Response::parse_packet(
            &packet[..=23],
            Some(IdentifyDeviceResponse::from(packet[24])),
        ))
    }
}
